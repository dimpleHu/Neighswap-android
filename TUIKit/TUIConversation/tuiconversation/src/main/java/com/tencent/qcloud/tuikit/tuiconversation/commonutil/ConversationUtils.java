...
public class ConversationUtils {
    ...
    public static ConversationInfo convertV2TIMConversation(final V2TIMConversation conversation) {
        ...
        if (isGroup) {
            ...
            File localAvatarFile = getLocalAvatarFile(userId);
            if (localAvatarFile != null && localAvatarFile.exists()) {
                // 本地头像存在，添加File对象
                faceList.add(localAvatarFile);
            } else if (TextUtils.isEmpty(conversation.getFaceUrl())) {
                // 无本地头像+无服务器URL，使用默认头像
     ...
     */
    private static File getLocalAvatarFile(String userId) {
